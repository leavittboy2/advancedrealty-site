...
if ((getBoolOptions($unit, 'unitListProp'))) {
  $rmBtns.= "<a class=\"bg-color2 bg-color1-hover font2\" href=\"/Property?pid=".$unit->propID."\">View Property</a>";
}
if ((getBoolOptions($unit, 'unitListDetail')))  {
  ...
  $imagesHTML = "";
   // foreach($unit->images as $image){
    $imageUrl = (isset($unit->images) && is_array($unit->images) && !empty($unit->images[0]) && isset($unit->images[0]->url)) ? $unit->images[0]->url : $fallback;
    $imagesHTML .= createSinglePhoto($unit, $imageUrl, $fallback, 'unit listing', NULL, NULL);
            
        
   // }
    $daysToAdd = 0;
    $dateAvailable = displayAvailability($unit->availDate, "Now", $daysToAdd);
    $availability = $dateAvailable; // Define the variable that's used in the HTML data attribute
    $amenities = implode( ",", (array)$unit->amenities);

$autoHTML.=<<<HTML
        <div class="rmwb_listing-wrapper" data-show="no" data-availability="{$availability}" data-unittype="{$unit->unitType}" data-rent="{$unit->marketrent}"  data-street="{$unit->street}" data-uid="{$unit->unitID}" data-availabledate="{$unit->availDate}" data-bathrooms="{$unit->bathrooms}" data-bedrooms="{$unit->bedrooms}">
       
        <div class="rmwb_header-section rmwb_section">
            <div class="rmwb_main-header">
            <h2 class="rmwb_listing_header">{$unit->street}</h2>
            <h3 class="rmwb_listing_header">{$unit->csz}</h3>
            </div>
        </div>
        ...
        {$imagesHTML}
         ...
         <ul class="rmwb_info-list">
                {$fn(createListItem('Expected Move Out Date', $unit->emoDate))}
